...
class CpageController extends Controller {
	...
	protected $acls = array ('data' => 'r:cms/cpage','index' => 'r:cms/cpage','add' => 'c:cms/cpage','edit' => 'u:cms/cpage','save' => 'id|u:cms/cpage;c:cms/cpage' );
	...
	public function save() {
		...
		if ($page) {
			...
			if ($id) {
				...
				$html [] = '[<a href="#' . tourl ( 'cms/cpage', false ) . '" onclick="nUI.closeAjaxDialog()">返回列表</a>] &nbsp;';
				$html [] = '[<a href="javascript:void(0);" onclick="return modify_current_page();">修改</a>] &nbsp;';
				$url = dbselect ()->from ( '{cms_page}' )->where ( array ('id' => $id ) )->get ( 'url' );
				...
				$html [] = '</p>';
				return NuiAjaxView::dialog ( implode ( '', $html ), '保存完成!', array ('model' => true,'height' => 'auto','func' => 'pageSaved','page' => $page ) );
			} else {
	...
	public function data($_cp = 1, $_lt = 20, $_sf = 'PG.id', $_od = 'd', $_ct = 0) {
		...
		if ($keywords) {
			...
			$where [] = array ('search_index MATCH' => $keywords,'||PG.title LIKE' => $t );
		}
		$rows->where ( $where );
		$rows->sort ( $_sf, $_od );
