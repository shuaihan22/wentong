...
function hook_for_do_admin_layout_admin($layout) {
	...
	if (icando ( 'm:cms' )) {
		...
		if (icando ( 'c:cms/page' )) {
			...
			$models = dbselect ( 'refid,name,role' )->from ( '{cms_model}' )->where ( array ('deleted' => 0,'is_topic_model' => 0,'hidden' => 0,'creatable' => 1 ) )->asc ( 'upid' );
		...
		if (icando ( 'c:cms/topic' )) {
			...
			$topics = dbselect ( 'refid,name,role' )->from ( '{cms_model}' )->where ( array ('deleted' => 0,'is_topic_model' => 1,'hidden' => 0,'creatable' => 1 ) )->asc ( 'upid' );
